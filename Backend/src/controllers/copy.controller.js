...
const addCopies = async (req, res, next) => {
  try {
    ...
    const results = await addBookCopies(bookId, copyIds, loggedUserId);
...
const updateStatus = async (req, res, next) => {
  try {
    ...
    const updated = await updateCopyStatus(copyId, status, loggedUserId);
...
const deleteCopyHandler = async (req, res, next) => {
  try {
    const { copyId } = req.params;
    const loggedUserId = req.user?.id;
    await deleteCopy(copyId, loggedUserId);
