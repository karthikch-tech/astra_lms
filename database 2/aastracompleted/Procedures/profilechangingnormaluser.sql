...
profileBlock: BEGIN
	...
	DECLARE v_first_part VARCHAR(50);
	DECLARE v_last_part VARCHAR(50);
    ...
    IF v_user_id IS NULL THEN
        SET v_error_msg = CONCAT(v_error_msg, 'User not found. ');
    END IF;
...
INTO v_current_username, v_current_email, v_current_full_name
FROM users
WHERE user_id = v_user_id;
	...
	-- Check if all fields are NULL or same as existing
IF 
(
...
THEN
    SET v_error_msg = CONCAT(
        v_error_msg,
        'All details are same. Profile not updated. '
    );
END IF;
    ...
    -- Update Full Name (First name mandatory)
...
END IF;
    ...
    SELECT 'Profile updated successfully.' AS status_message;
